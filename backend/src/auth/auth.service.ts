...
export class AuthService {
    ...
    try {
      ...
      if (!user) {
        throw new NotFoundException(
          'Email not found. Please register first or try again',
        );
      }
  ...
  async updateProfile(req: any, dto: UpdateProfileDto) {
    ...
    try {
      ...
      if (imageUrl) {
        const result =
          await this.cloudinaryService.uploadImageFromUrl(imageUrl);
    ...
    try {
      ...
      if (isSame) {
        throw new BadRequestException(
          'New password must be different with the current password',
        );
      }
  ...
  async forgotPassword(dto: ForgotPasswordDto) {
    ...
    try {
      ...
      if (!user) {
        return {
          message:
            'Nếu mailer tồn tại trong hệ thống, chúng tôi đã gửi hướng dẫn khôi phục mật khẩu.',
        };
      }
